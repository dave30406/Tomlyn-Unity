...
namespace Tomlyn.Model
{
    internal class ModelToTomlTransform
    {
        ...
        {
            public ObjectPath(string name, bool isTableArray)
            {
                Name = name;
                IsTableArray = isTableArray;
            }

            public string Name { get; }
            public bool IsTableArray { get; }

            public override bool Equals(object obj)
            {
                return obj is ObjectPath path &&
                       Name == path.Name &&
                       IsTableArray == path.IsTableArray;
            }

            public override int GetHashCode()
            {
                return HashCode.Combine(Name, IsTableArray);
            }
        }
        
        private readonly struct KeyValueAccessor
        {
            public KeyValueAccessor(string key, object value, DynamicAccessor accessor)
            {
                Key = key;
                Value = value;
                Accessor = accessor;
            ...
            public override bool Equals(object obj)
            {
                return obj is KeyValueAccessor accessor &&
                       Key == accessor.Key &&
                       EqualityComparer<object>.Default.Equals(Value, accessor.Value) &&
                       EqualityComparer<DynamicAccessor>.Default.Equals(Accessor, accessor.Accessor);
            }

            public override int GetHashCode()
            {
                return HashCode.Combine(Key, Value, Accessor);
            }
        }
